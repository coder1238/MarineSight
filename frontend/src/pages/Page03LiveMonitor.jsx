...
export default function Page03LiveMonitor({ onNavigate }) {
  ...
  // 3. Target Inspection & Fleet State
  ...
  // 5. Streaming NMEA Telemetry Console
  ...
  const playSound = (type = 'ping') => {
    ...
    try {
      ...
      if (type === 'ping') {
        ...
        // High-frequency burst mimicking VHF PTT click
        osc.type = 'square';
  ...
  const simPhysics = useMemo(() => {
    return getFullSimulationPhysicsState(activeIncident, simHour, {
      ...
      particleCount: 220 // Lightweight for smooth 60fps live performance
    });
  ...
  useEffect(() => {
    ...
    const speedRates = {
      "0.5x": 0.15, // 0.15 hours per real second
      "1x": 0.45,   // 0.45 hours per real second
      "2x": 1.2,    // 1.2 hours per real second
      "5x": 3.0     // 3.0 hours per real second
    };
  ...
  // Candidate vessels for fleet directory
  const candidateFleet = activeIncident?.candidateVessels || [activeIncident?.topVessel];

  return (
    <div className="p-3 sm:p-5 space-y-3 flex flex-col h-[calc(100vh-65px)] overflow-hidden bg-background-light">
      ...
      <div className="flex-1 grid grid-cols-1 lg:grid-cols-12 gap-3 min-h-0">
        ...
        <div className="lg:col-span-8 xl:col-span-9 bg-white border border-border-marine rounded-2xl p-2.5 shadow-marine-sm flex flex-col min-h-0 relative overflow-hidden">
          ...
          <div className="flex-1 min-h-0 rounded-xl overflow-hidden border border-border-marine/60 relative">
            <GISRealMap 
              mode="live" 
        ...
        {/* Right Section: Target Inspection HUD, Fleet Directory & Maritime Alerts (Col 4 or 3) */}
        <div className="lg:col-span-4 xl:col-span-3 flex flex-col gap-3 min-h-0">
          ...
          {selectedVessel ? (
            ...
            /* Fleet Radar Directory (When no vessel is selected) */
            <div className="bg-white border border-border-marine rounded-2xl p-3 shadow-marine-sm flex flex-col gap-2">
              <div className="flex items-center justify-between pb-1.5 border-b border-border-marine">
                ...
                <span className="text-[10px] font-mono px-1.5 py-0.5 rounded bg-ocean-sky text-ocean font-bold">
                  {candidateFleet.length} Tracked
                </span>
              </div>

              <p className="text-[11px] text-text-secondary">
                Select a vessel to inspect kinematic telemetry, dispatch interceptors, or hail over VHF:
              </p>
                    ...
                    >
                      <div>
                        ...
                        <div className="text-[10px] font-mono text-text-muted mt-0.5">
                          MMSI: {v.mmsi} · {v.speedKn || 12} kn · {v.heading || 284}°
                        </div>
                      </div>
          ...
          <div className="flex-1 bg-white border border-border-marine rounded-2xl p-3 shadow-marine-sm flex flex-col min-h-0 overflow-hidden">
                  ...
                  >
                    ...
                    <div className="mt-2 pt-1.5 border-t border-border-marine/40 flex items-center justify-between text-[10px]">
                      ...
                      <button
                        ...
                        title={alt.acknowledged ? "Marked as Acknowledged" : "Click to Acknowledge"}
                      >
                        <Check className="w-2.5 h-2.5" />
                        <span>{alt.acknowledged ? "Acknowledged" : "Acknowledge"}</span>
                      </button>

                      {/* Investigate / Deep Nav */}
                      <button 
                        onClick={(e) => {
                          e.stopPropagation();
            ...
            {/* Radar Receiver & Signal Quality Footer */}
            <div className="mt-2 pt-2 border-t border-border-marine text-[10px] text-text-muted font-mono flex items-center justify-between">
