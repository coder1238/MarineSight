 ...
 */
export function calculateWeathering(hoursElapsed, apiGravity = 31.5) {
  const t = Math.max(0, hoursElapsed);
  
  // Evaporation asymptotic curve
  const evaporated = Math.min(38, Math.round(12 + (1 - Math.exp(-t / 18)) * 20));
  
  // Emulsification water uptake
  const emulsified = Math.min(65, Math.round(15 + (1 - Math.exp(-t / 22)) * 42));
  
  // Natural vertical column dispersion
  const dispersed = Math.min(22, Math.round(4 + (1 - Math.exp(-t / 32)) * 14));
  
  // Remaining surface slick mass fraction
  const surface = Math.max(12, 100 - evaporated - dispersed);
  
  // Viscosity growth (cSt)
  const viscosityCst = Math.round(180 * Math.exp(0.045 * t));
...
export function getFullSimulationPhysicsState(incident, timestampOrHours = "T+72", userOptions = {}) {
  ...
  const currentSpeed = (incident.environment?.currentSpeedMs ? incident.environment.currentSpeedMs * 1.94384 : 0.95) * currentMultiplier;
  const currentHeading = incident.environment?.currentDirectionDeg || 75;
  const windSpeed = incident.environment?.windSpeedKn || 18.5;
  const windFromDeg = incident.environment?.windDirectionDeg || 295;
  ...
  const weathering = calculateWeathering(hoursElapsed);
