...
export default function TopHeader({ currentPage, setCurrentPage, onSearch }) {
  ...
  return (
    <header className="bg-white border-b border-border-marine px-4 py-2.5 flex items-center justify-between sticky top-0 z-20 shadow-marine-sm">
      {/* Left: Breadcrumbs, Region Selector & Quick Switcher */}
      <div className="flex items-center gap-2.5 flex-wrap">
        <div className="flex items-center text-xs text-text-secondary">
          <span className="font-semibold text-ocean-navy">MarineSight</span>
          <span className="mx-1.5 text-border-marine font-bold">/</span>
          <button 
            onClick={() => setCurrentPage("workspace")} 
            className="hover:text-ocean transition-colors font-mono font-bold text-ocean-deep"
          >
            {activeIncidentId}
          </button>
          <span className="mx-1.5 text-border-marine font-bold">/</span>
          <span className="text-ocean font-medium truncate max-w-[170px] sm:max-w-none">
        ...
        <div className="relative">
          <button
            ...
            className="flex items-center gap-1.5 bg-ocean-navy text-white px-2.5 py-1 rounded-lg text-xs font-mono font-semibold transition-all hover:bg-ocean shadow-sm"
            title="Switch Maritime Simulation Region"
          >
            <span>{activeIncident?.flagEmoji || "🇮🇳"}</span>
            <span className="hidden sm:inline">{activeIncident?.regionShort || "Arabian Sea"}</span>
            <span className="text-[10px] text-[#00E5FF] font-bold">({activeIncident?.spillAreaKm2} km²)</span>
            <ChevronDown className="w-3 h-3 text-white/80" />
          </button>

          {showRegionSwitcher && (
            <div className="absolute left-0 mt-1.5 w-80 bg-white border border-border-marine rounded-xl shadow-marine-lg py-1.5 z-50 max-h-96 overflow-y-auto">
        ...
        <div className="relative">
          <button 
            ...
            className="flex items-center gap-1.5 bg-ocean-sky/70 hover:bg-ocean-sky border border-ocean/20 text-ocean-deep px-2 py-1 rounded text-xs font-medium transition-all"
          >
            <span>Jump Page</span>
          ...
          {showPageSwitcher && (
            <div className="absolute left-0 mt-1.5 w-80 bg-white border border-border-marine rounded-lg shadow-marine-lg py-1.5 z-50 max-h-96 overflow-y-auto">
      ...
      <div className="flex items-center gap-3">
        {/* UTC Clock */}
        <div className="flex items-center gap-1.5 bg-ocean-light border border-border-marine px-2.5 py-1 rounded text-xs font-mono text-ocean-deep font-semibold">
          <Clock className="w-3.5 h-3.5 text-ocean" />
          <span>{utcTime || "14:45:00 UTC"}</span>
        </div>
        ...
        <div className="relative">
          ...
          {showNotifications && (
            <div className="absolute right-0 mt-2 w-80 bg-white border border-border-marine rounded-lg shadow-marine-lg p-2 z-50">
        ...
        {/* User Profile */}
        <div className="flex items-center gap-2 pl-2 border-l border-border-marine">
          <div className="w-7 h-7 rounded-full bg-ocean text-white flex items-center justify-center font-bold text-xs shadow-sm">
            EV
          </div>
